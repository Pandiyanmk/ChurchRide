...
class TimerRequestPage : AppCompatActivity(), RequestListener, OnMapReadyCallback {
    ...
    private fun splitBookingDetails(bookingData: String) {
        try {
            ...
            if (bookingData != null) {
                val jsonObject = JSONObject(bookingData)
                val rideId = jsonObject.getString("ride_id")
                val pickUpLat = jsonObject.getString("pickup_lat")
                val pickupLong = jsonObject.getString("pickup_long")
                val pickupAddress = jsonObject.getString("pickup_address")
                val dropLat = jsonObject.getString("drop_lat")
                val dropLong = jsonObject.getString("drop_long")
                val dropAddress = jsonObject.getString("drop_address")
                distanceTime = jsonObject.getString("distanceTime")
                distanceMiles = jsonObject.getString("distanceKM")
